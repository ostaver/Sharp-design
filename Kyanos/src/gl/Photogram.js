...
// real print, then lifts the specimens off and runs water down the paper as the page scrolls.
...
uniform float uWash;     // water front
...
void main() {
  ...
  // Water: a front that runs down the sheet and ripples what it has just passed.
  float wobble = (texture(uNoise, vec2(uv.x * 0.9 + 0.2, 0.3)).r - 0.5) * 0.22
               + (texture(uNoise, vec2(uv.x * 3.1, 0.6 + uTime * 0.01)).g - 0.5) * 0.06;
  float s = mix(-0.3, 1.3, uWash) - (1.0 - uv.y) + wobble;
  float wet = smoothstep(-0.015, 0.05, s);
  float ripple = smoothstep(0.0, 0.03, s) * exp(-max(s, 0.0) * 3.0) * (1.0 - smoothstep(0.82, 1.0, uWash));
  vec2 w = uv * asp * 2.2;
  float h0 = heightAt(w);
  ...
  float foam = exp(-abs(s) * 110.0) * step(0.001, uWash) * (1.0 - smoothstep(0.85, 1.0, uWash));
  col += foam * 0.22 * (0.6 + 0.4 * texture(uNoise, vec2(uv.x * 6.0, uTime * 0.3)).g);
  ...
  vec2 luv = (vUv - 0.5) / (1.0 + uLift * 0.04 + (1.0 - uPlace) * 0.03) + 0.5;
...
export class Photogram {
  ...
  // The lamp follows a mouse over the sheet, or a finger pressed to it.
  steer(r, dt, running) {
  ...
  bake() {
    ...
    this.bakeProgram.uniforms.uLight.value = this.read.texture;
    this.stage.renderer.render({ scene: this.bakeMesh, target: this.baked });
